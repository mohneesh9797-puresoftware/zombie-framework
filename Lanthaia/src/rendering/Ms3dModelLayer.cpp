...
Ms3dModelLayer::Ms3dModelLayer(zfw::IBroadcastHandler& bh, zfw::IEngine& engine, zfw::IEntityWorld2& world, zfw::IResourceManager2& resMgr,
        RenderingKit::IRenderingManager& rm)
    : renderables([this](auto&& ...args) { UpdateInPlace(args...); }),
      ms3dModelResourceProvider(engine, rm),
    world(world), resMgr(resMgr) {
...
void Ms3dModelLayer::OnComponentEvent(zfw::IEntityWorld2& world, intptr_t entityId, zfw::IComponentType &type, void *data, zfw::ComponentEvent event) {
    if (&world != &this->world) {
        return;
    }
...
void Ms3dModelLayer::Render(RenderingContext const& ctx) {
    for (auto const& [entityId, renderable] : renderables.entities) {
        ...
        auto transform = glm::translate(mat4x4(1.0f), position.pos) * glm::mat4_cast(position.rotation) * glm::scale({}, position.scale);
...
void Ms3dModelLayer::UpdateInPlace(zfw::IEntityWorld2& world, zfw::EntityId entityId, decltype(renderables)::References components, Ms3dModel*& model) {
