...
void TerrainRenderLayer::OnComponentEvent(
...
void TerrainRenderLayer::Render(RenderingContext const& ctx) {
    ...
    for (auto& [entityId, renderable] : renderables.entities) {
        ...
        if (!rwm.gc) {
            ...
            auto heightMapRes = resMgr.GetResourceByPath<Obs::Res::ResourceHeightMap>(
                terrain.path, zfw::IResourceManager2::kResourceRequired);
            ...
            char* materialRecipe = zfw::Params::BuildAlloc(2, "shader", "path=RenderingKit/basicTextured",
                "texture:tex", ("path=" + terrain.texture + ",wrapx=repeat,wrapy=repeat").c_str());

            rwm.material = resMgr.GetResource<RenderingKit::IMaterial>(
        ...
        rm.DrawPrimitivesTransformed(
            rwm.material, RenderingKit::RK_TRIANGLES, rwm.gc.get(), glm::translate(mat4x4(1.0f), position.pos));
    }
}

void TerrainRenderLayer::UpdateInPlace(zfw::IEntityWorld2& world, zfw::EntityId entityId,
    decltype(renderables)::References components, RenderableWorldMesh& mesh) {
}
